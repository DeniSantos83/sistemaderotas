...
export default function SectorListPage() {
  ...
  async function loadSectors(showRefreshState = false) {
    try {
      setError("");
      ...
      if (trimmedSearch) {
        query = query.or(
          `name.ilike.%${trimmedSearch}%,code.ilike.%${trimmedSearch}%,description.ilike.%${trimmedSearch}%`
        );
      }
  ...
  const stats = useMemo(() => {
    ...
    const active = sectors.filter((item) => item.active).length;
    const inactive = sectors.filter((item) => !item.active).length;
    const withCode = sectors.filter((item) => normalizeText(item.code)).length;
    ...
    <div style={styles.page}>
      <section style={styles.heroCard}>
        ...
        <div style={styles.heroContent}>
          ...
          <h1 style={styles.heroTitle}>Gestão de setores</h1>

          <p style={styles.heroText}>
            Organize os setores da sua operação, acompanhe códigos e descrições
            e mantenha a estrutura organizacional clara dentro de uma
        ...
        <div style={styles.heroActions}>
          <Link to="/setores/novo" style={styles.primaryButton}>
            <Plus size={18} />
      ...
      <section style={styles.statsGrid}>
        <article style={styles.statCard}>
          ...
          <p style={styles.statDetail}>Setores cadastrados no sistema</p>
        </article>
      ...
      <section style={styles.panelCard}>
        <div style={styles.panelHeader}>
          <div>
            ...
            <h3 style={styles.panelTitle}>Lista de setores</h3>
            <p style={styles.panelText}>
              Filtre por nome, código, descrição ou status para localizar
              rapidamente qualquer setor cadastrado.
            </p>
          </div>
        ...
        <div style={styles.filtersSummary}>
          <div style={styles.filtersGroup}>
            <span style={styles.infoChip}>
              <Building2 size={14} />
              <span>{sectors.length} resultado(s)</span>
            </span>
        ...
        {loading ? (
          <div style={styles.emptyPanel}>Carregando setores...</div>
        ) : sectors.length === 0 ? (
          <div style={styles.emptyPanel}>Nenhum setor encontrado.</div>
        ) : (
              ...
              <tbody>
                {sectors.map((sector) => (
                  <tr
                    ...
                    style={{
                      ...styles.row,
                      ...(hoveredRow === sector.id ? styles.rowHover : {}),
                    }}
                  >
                    <td style={styles.td}>
                      <div style={styles.primaryText}>{sector.name || "-"}</div>
                    ...
                    <td style={styles.td}>
                      <div style={styles.primaryText}>{sector.code || "-"}</div>
                      <div style={styles.secondaryText}>Identificação interna</div>
                    </td>

                    <td style={styles.td}>
                      <span style={getStatusStyle(sector.active)}>
                        {getStatusLabel(sector.active)}
                      </span>
                    </td>
