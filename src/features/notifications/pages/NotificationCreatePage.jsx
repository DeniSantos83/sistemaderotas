...
export default function NotificationCreatePage() {
  ...
  useEffect(() => {
    async function loadDependencies() {
      try {
        const [{ data: usersData, error: usersError }, { data: recordsData, error: recordsError }] =
          await Promise.all([
        ...
        setError(err.message || "Não foi possível carregar os dados auxiliares.");
      }
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(err.message || "Não foi possível salvar a notificação.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="form-page">
      <section className="panel-card">
        <div className="form-header">
          <div>
            <p className="hero-kicker">Nova comunicação</p>
            <h1 className="form-title">Cadastrar notificação</h1>
            <p className="form-subtitle">
              Crie um aviso interno para um usuário e, se quiser, vincule a um registro.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="premium-form">
          <div className="form-grid">
            ...
            <div className="form-group form-group-full">
              ...
              <input
                ...
                value={form.title}
                onChange={handleChange}
                placeholder="Ex.: Registro pendente de sincronização"
                required
              />
            </div>
          ...
          <div className="form-actions">
            ...
            >
              <Save size={16} />
              <span>{submitting ? "Salvando..." : "Salvar notificação"}</span>
            </button>
          </div>
        </form>
      </section>
